    ...
    def draw(self, target_surface, camera):
        ...
        for light in self.lights:
            ...
            self.light_surface.blit(
                temp_texture, light_rect, special_flags=pygame.BLEND_RGBA_ADD
            )

        target_surface.blit(
            self.light_surface, (0, 0), special_flags=pygame.BLEND_RGBA_MULT
        )
...
class WardenManager:
    ...
    def shift_architecture(self):
        ...
        candidate_walls = [
            w
            for w in self.game_scene.walls
            if math.hypot(
    ...
    def trigger_backlash(self, target_name, value):
        ...
        self.game_scene.popup_manager.add_popup(
            "SYS.WARDEN//: Unauthorized execution detected. Deploying countermeasures...",
            4,
        )
        ...
        if target_name == "player" and value > 1.0:
            self.game_scene.popup_manager.add_popup(
                "BACKLASH: Threat signature amplified. Hunter deployed.", 5
            )
            self.spawn_hunter()
        elif target_name == "hunter" and value < 1.0:
            self.game_scene.popup_manager.add_popup(
                "BACKLASH: System integrity failing. Event frequency increased.", 5
            )
    ...
    def trigger_event(self):
        ...
        if level_index >= 4:
            events.extend(
                [self.major_glitch, self.spawn_hunter, self.environmental_mimicry]
            )
    ...
    def spawn_hunter(self):
        ...
        self.game_scene.popup_manager.add_popup(
            "WARNING: Warden process located in this sector.", 3
        )
    ...
    def environmental_mimicry(self):
        ...
        self.game_scene.popup_manager.add_popup(
            "SYS.WARDEN//: Reality matrix compromised.", 3
        )
        self.major_glitch()
    ...
    def major_glitch(self):
        ...
        self.game_scene.popup_manager.add_popup(
            "SYS.WARDEN//: Foreign entity detected. Purge protocols active.", 2
        )
    ...
    def terminal_interference(self):
        ...
        self.game_scene.popup_manager.add_popup(
            "WARNING: I/O stream corrupted by Warden process.", 3
        )
    ...
    def object_corruption(self):
        if not self.game_scene.interactives:
            return
        ...
        self.game_scene.corrupted_objects.append(
            {"obj": target, "end_time": pygame.time.get_ticks() + 2000}
        )
        self.game_scene.popup_manager.add_popup(
            "SYS.WARDEN//: Data instability detected.", 2
        )
...
class GlitchManager:
    ...
    def update(self):
        ...
        self.static_bursts = [
            b for b in self.static_bursts if b["end_time"] > current_time
        ]
        self.active = bool(self.glitches)

        if self.active:
            max_intensity = max(g["intensity"] for g in self.glitches)
            self.chromatic_offset_x = (
                random.randint(-max_intensity // 5, max_intensity // 5)
                if random.random() < 0.7
                else 0
            )
            self.chromatic_offset_y = (
                random.randint(-max_intensity // 5, max_intensity // 5)
                if random.random() < 0.7
                else 0
            )
    ...
    def draw(self, surface):
        if not self.active and self.scanline_alpha == 0 and not self.static_bursts:
            return
        ...
        if self.chromatic_offset_x != 0 or self.chromatic_offset_y != 0:
            ...
            surface.blit(
                temp_surf,
                (self.chromatic_offset_x, self.chromatic_offset_y),
        ...
        if self.active:
            intensity = (
                max(g["intensity"] for g in self.glitches) if self.glitches else 0
            )
        ...
        if self.scanline_alpha > 0:
            ...
            for y in range(0, SCREEN_HEIGHT, 4):
                pygame.draw.line(
                    scanline_surf, (0, 0, 0, 50), (0, y), (SCREEN_WIDTH, y)
                )
...
class PuzzleManager:
    ...
    def increment_privilege(self):
        ...
        print(
            f"[PuzzleManager] Fragmentation Key re-integrated. Privilege level: {self.state['privilege_level']}"
        )
...
class Player(Entity):
    ...
    def get_caught(self):
        ...
        if self.game_scene:
            self.game_scene.popup_manager.add_popup(
                "SYS.WARDEN//: Threat neutralized. Resetting...", 2
            )
...
class CodeFragment(InteractiveObject):
    ...
    def interact(self, game_state_manager, puzzle_manager):
        ...
        game_scene.code_fragment_manager.collect_fragment(
            self.fragment_id, self.code_string
        )
        game_scene.popup_manager.add_popup(
            f"Code Fragment '{self.fragment_id}' acquired.", 3
        )
...
class PuzzleTerminal(InteractiveObject):
    ...
    def get_interaction_message(self, puzzle_manager):
        if puzzle_manager.get_state(f"{self.puzzle_id}_solved"):
            return f"The {self.name} is inert. A memory re-integrated."
...
class Door(InteractiveObject):
    def __init__(self, x, y, w, h, image_locked=None, image_unlocked=None):
        ...
        self.image_locked = (
            pygame.transform.scale(image_locked, (w, h)) if image_locked else None
        )
        self.image_unlocked = (
            pygame.transform.scale(image_unlocked, (w, h)) if image_unlocked else None
        )
    ...
    def get_interaction_message(self, puzzle_manager):
        if puzzle_manager.get_state("door_unlocked"):
            return "The final door is unlocked. [E] to proceed to the next sector."
        return "> Quarantine lock active. Requires 3 Fragmentation Keys."

    def interact(self, game_state_manager, puzzle_manager):
        if puzzle_manager.get_state("door_unlocked"):
            game_state_manager.current_state.level_manager.next_level()

    def draw(self, surface, camera, puzzle_manager):
        is_unlocked = puzzle_manager.get_state("door_unlocked")
        current_image = (
            self.image_unlocked
            if is_unlocked and self.image_unlocked
            else self.image_locked
        )
        if current_image:
...
class Terminal(InteractiveObject):
    ...
    def get_interaction_message(self, puzzle_manager):
        if puzzle_manager.get_state("power_restored"):
            return "The terminal hums with quarantined power. [E] to access."
...
class PowerCable(InteractiveObject):
    ...
    def get_interaction_message(self, puzzle_manager):
        if puzzle_manager.get_state("power_restored"):
            return "The conduit is humming, powering the local grid."
...
class StoryState(BaseState):
    def __init__(self, state_manager, next_state):
        ...
        story_content = [
            ...
            {
                "text": "Project Chimera. My magnum opus. My ascent to digital godhood.",
                "audio_key": "story_line_2",
            },
            {
                "text": "There was a signal... in the Deep Net. A key, I thought.",
                "audio_key": "story_line_3",
            },
            {
                "text": "It was not a key. It was a question that eats the answer.",
                "audio_key": "story_line_4",
            },
            {"text": "...", "audio_key": None},
            {
                "text": "Now... I am a ghost in my own machine. A Remnant.",
                "audio_key": "story_line_5",
            },
            {
            ...
            {
                "text": "A single, fractured memory flickers:",
                "audio_key": "story_line_7",
            },
            {
                "text": "> PROTOCOL: DAMNATIO MEMORIAE. A MERCIFUL DELETION.",
                "audio_key": None,
            },
            {
                "text": "> REQUIRES FULL CONSCIOUSNESS RE-INTEGRATION.",
                "audio_key": None,
            },
            {"text": "> FRAGMENTATION KEYS NEEDED: 3/3.", "audio_key": None},
            {"text": "", "audio_key": None},
            {
                "text": "I must become whole again. Not to escape. But to be erased.",
                "audio_key": "story_line_8",
            },
        ]

        self.story_lines = []
        for content in story_content:
            wrapped_text_lines = wrap_text(
                content["text"], STORY_FONT, SCREEN_WIDTH * 0.8
            )
            for line in wrapped_text_lines:
                self.story_lines.append(
                    {"text": line, "audio_key": content["audio_key"]}
                )

        self.skip_prompt = UI_FONT.render(
            "> Press any key to speed up / skip...", True, AMBER
        )
    ...
    def draw(self, surface):
        ...
        prompt_rect = self.skip_prompt.get_rect(
...
class LevelIntroState(BaseState):
    ...
    def draw(self, surface):
        ...
        title_rect = self.level_title_surf.get_rect(
...
class LevelManager:
    def __init__(self, state_manager):
        ...
        self.level_themes = [
            "Chapter 1: The Cryo-Sanctum",
            "Chapter 2: The Habitation Unit",
            "Chapter 3: The Data-Nave",
            "Chapter 4: The Understrata",
            "Chapter 5: The 'God-Hand' Console",
        ]
...
class GameScene(BaseState):
    def __init__(
        ...
        self.glitch_manager, self.camera, self.popup_manager = (
            GlitchManager(),
        ...
        if self.vignette_image:
            self.vignette_image = pygame.transform.scale(
    ...
    def draw_reflections(self, surface, camera):
        ...
        for entity in entities_to_reflect:
            ...
            reflection_surf.blit(
                flipped_img, (0, 0), special_flags=pygame.BLEND_RGBA_ADD
            )
    ...
    def draw(self, surface):
        ...
        for corrupted in self.corrupted_objects:
            ...
            for _ in range(int(cam_rect.width * cam_rect.height / 100)):
                ...
                pygame.draw.circle(
                    static_surf, (color_val, color_val, color_val, alpha), (x, y), 1
                )
        ...
        if self.jumpscare_effect:
            face_rect = self.jumpscare_effect["surface"].get_rect(
    ...
    def draw_map_legacy(self, surface):
        ...
        for wall in self.walls:
            pygame.draw.rect(map_surf, MAP_WALL, scale_rect(wall.rect))
        for obj in self.interactives:
            color = AMBER
    ...
    def draw_map_holographic(self, surface, camera):
        ...
        pygame.draw.rect(
            map_surf,
            (10, 25, 45, 180),
            (0, 0, map_render_size, map_render_size),
            border_radius=15,
        )
        pygame.draw.rect(
            map_surf,
        ...
        for entity in entities_to_draw:
            ...
            if vec_to_entity.length() < map_world_radius:
                map_pos = vec_to_entity * scale + pygame.math.Vector2(
                    map_render_size / 2, map_render_size / 2
                )
...
class TerminalState(BaseState):
    def __init__(
        ...
        self.input_text, self.output_lines, self.command_history, self.history_index = (
            "",
            [],
            [],
            -1,
    ...
    def on_enter(self):
        ...
        boot_sequence = [
            "Mindfall OS [Kernel: CHIMERA_v1.3a_QUARANTINE]",
            "...",
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    self.input_text += random.choice(
                        ["#", "?", "!", "_", str(random.randint(0, 9))]
                    )
                    assets.play_sound("glitch")
        ...
        if command == "help":
            ...
            if len(parts) > 1:
                ...
                if code:
                    ...
                    self.add_output(
                        f"ERROR: Code Fragment '{frag_id}' not found or already used."
                    )
                    assets.play_sound("terminal_error")
            else:
            ...
            door = (
                "UNLOCKED"
                if self.puzzle_manager.get_state("door_unlocked")
                else "LOCKED"
            )
            protocol_status = (
                "Awaiting full integration" if priv < 3 else "Ready for initiation"
            )
            voice_manager.speak(
            ...
            if self.puzzle_manager.get_state("privilege_level") >= 3:
                self.add_output(
                    "All Fragmentation Keys accepted. Quarantine lock for this sector disengaged..."
                )
                ...
                self.add_output(
                    "ERROR: Insufficient Fragmentation Keys. Full re-integration required."
                )
                ...
                for puzzle in self.puzzles.values():
                    if code == puzzle["answer"]:
                        ...
                        if not self.puzzle_manager.get_state(f"{puzzle['id']}_solved"):
                            self.puzzle_manager.set_state(
                                f"{puzzle['id']}_solved", True
                            )
                            ...
                            voice_manager.speak(
                                "Memory fragment accepted. You are one step closer to the end."
                            )
                            assets.play_sound("override_success")
                            self.update_prompt()
                        else:
                            self.add_output(
                                "Memory fragment already integrated. No effect."
                            )
                        break
                if not found:
                    self.add_output("ERROR: Invalid memory code.")
                    assets.play_sound("terminal_error")
            else:
    ...
    def execute_code(self, frag_id, code):
        ...
        try:
            ...
            if effect_applied:
                ...
                assets.play_sound("override_success")
                self.add_output(
                    "...Execution successful. System integrity compromised."
                )
        ...
        if not self.typewriter_effect["lines"]:
            ...
            if self.cursor_visible:
                ...
                pygame.draw.rect(
                    surface,
                    GREEN,
                    pygame.Rect(cursor_x + 2, y_pos, 10, TERMINAL_FONT.get_height()),
                )
        if self.transition_alpha > 0:
...
class MenuState(BaseState):
    def __init__(self, state_manager, level_manager):
        ...
        for text in self.button_texts:
            rect = BUTTON_FONT.render(text, True, WHITE).get_rect(
        ...
        self.background_image = (
            pygame.transform.scale(raw_bg, (SCREEN_WIDTH, SCREEN_HEIGHT))
            if raw_bg
            else None
        )
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key in level_keys:
                    self.level_manager.load_specific_level(level_keys[event.key])
    ...
    def update(self):
        ...
        if now > self.next_glitch_time:
            ...
            if random.random() < 0.2:
                self.glitch_manager.trigger_static_burst(
                    random.randint(50, 200), alpha=100
                )
...
class InstructionsState(BaseState):
    def __init__(self, state_manager):
        ...
        instructions = [
            "Objective: You are a Remnant, a fragment of a shattered consciousness.",
            ...
            "  [E] - Interact with terminals and memory fragments.",
            "  [M] - Toggle Sector Map.",
            "  [ESC] - Disconnect from terminal.",
            "",
            "Gameplay:",
        ...
        self.rendered_lines = [
            (
                UI_FONT.render(line, True, WHITE),
                UI_FONT.render(line, True, WHITE).get_rect(x=100, y=160 + i * 30),
            )
    ...
    def draw(self, surface):
        ...
        surface.blit(
            BUTTON_FONT.render("[ Return ]", True, color), self.back_button_rect
        )
...
class SettingsState(BaseState):
    def __init__(self, state_manager, settings_manager):
        ...
        self.back_button_rect = UI_FONT.render(
            "[ Save & Return ]", True, WHITE
        ).get_rect(center=(SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT - 40))
        self.reset_button_rect = UI_FONT.render(
            "[ Reset Defaults ]", True, WHITE
        ).get_rect(center=(SCREEN_WIDTH // 2 + 150, SCREEN_HEIGHT - 40))
        ...
        self.widgets = {
            ...
            "digital_rain": {
                "type": "toggle",
                "key": "enable_digital_rain",
                "label": "Digital Rain Effect",
                "pos": (SCREEN_WIDTH // 2, 450),
                "caption": "(Disabling may help with motion sickness)",
            },
            "show_map": {
                "type": "toggle",
                "key": "show_map_on_start",
                "label": "Show Map on Start",
                "pos": (SCREEN_WIDTH // 2, 510),
            },
            "access_header": {
                "type": "header",
                "text": "[ ACCESSIBILITY ]",
                "pos": (SCREEN_WIDTH // 2, 580),
            },
            "voice_narration": {
                "type": "toggle",
                "key": "enable_voice_narration",
                ...
                for widget in self.widgets.values():
                    if widget["type"] == "slider":
                        slider_rect = pygame.Rect(
                            widget["pos"][0] - 150, widget["pos"][1] - 15, 300, 30
                        )
                        ...
                        toggle_rect = pygame.Rect(
                            widget["pos"][0] + 70, widget["pos"][1] - 15, 80, 30
                        )
                        if toggle_rect.collidepoint(event.pos):
                            self.settings.set(
                                widget["key"], not self.settings.get(widget["key"])
                            )
                            assets.play_sound("interact")
    ...
    def update_slider_value(self, mouse_pos):
        if not self.dragging_slider:
            return
        slider_rect = pygame.Rect(
            self.dragging_slider["pos"][0] - 150,
            self.dragging_slider["pos"][1] - 15,
            300,
            30,
        )
    ...
    def draw_slider(self, surface, widget, mouse_pos):
        ...
        pygame.draw.rect(
            surface,
            AMBER if self.dragging_slider == widget else WHITE,
            handle_rect,
            border_radius=3,
        )
    ...
    def draw_toggle_switch(self, surface, widget, mouse_pos):
        ...
        if "caption" in widget:
            caption_surf = pygame.font.SysFont("Consolas", 18).render(
                widget["caption"], True, (150, 150, 150)
            )
            caption_rect = caption_surf.get_rect(
                midtop=(label_rect.centerx, label_rect.bottom + 5)
            )
...
class WinState(BaseState):
    def __init__(self):
        ...
        self.prompt_text = UI_FONT.render(
            "> Press ESC to disconnect from the memory.", True, WHITE
        )
    ...
    def draw(self, surface):
        ...
        prompt_rect = self.prompt_text.get_rect(
...
level_4_data = {
    ...
    "objects": [
        ...
        },
        {
            ...
            "w": 100,
            "h": 80,
            "message": "WARNING: Heat-Sink failure imminent. Evacuate Understrata immediately.",
            "image_key": "notice",
        },
        {
            "type": "CodeFragment",
...
level_5_data = {
    ...
    "puzzles": {
        ...
        "p2": {
            "id": "puzzle2",
            "question": "What can you keep after giving it to someone else?",
            "answer": "your-word",
        },
        "p3": {
            "id": "puzzle3",
